...
- (NSString *)gtm_stringByEscapingForHTML {
    ...
    for (NSUInteger i = 0; i < self.length; ++i) {
        HTMLEscapeMap *val = bsearch(&buffer[i], gUnicodeHTMLEscapeMap,
                                     sizeof(gUnicodeHTMLEscapeMap) / sizeof(HTMLEscapeMap),
        ...
        if (length > 3 && length < 11) {
            if ([escapeString characterAtIndex:1] == '#') {
                ...
                for (unsigned i = 0; i < sizeof(gAsciiHTMLEscapeMap) / sizeof(HTMLEscapeMap); ++i) {
                    NSString *escapeSequence = [NSString stringWithCString:gAsciiHTMLEscapeMap[i].escapeSequence encoding:NSUTF8StringEncoding];
                    if ([escapeString isEqualToString:escapeSequence]) {
