...
+ (UIButton *)glossyButtonWithTitle:(NSString *)title color:(UIColor *)color highlightColor:(UIColor *)highlightColor  {
  return [self glossyButtonWithTitle:title color:color highlightColor:highlightColor withArrow:NO];
}
...
+ (UIButton *)glossyButtonWithTitle:(NSString *)title color:(UIColor *)color highlightColor:(UIColor *)highlightColor withArrow:(BOOL)arrow {
  ...
  nextButton.frame = CGRectMake(kButtonMargin, 0, screenWidth - kButtonMargin*2, kButtonHeight);
  nextButton.autoresizingMask = UIViewAutoresizingFlexibleWidth;
  
  // Configure background image(s)
  [nextButton setBackgroundToGlossyRectOfColor:color withBorder:YES withArrow:arrow forState:UIControlStateNormal];
  [nextButton setBackgroundToGlossyRectOfColor:highlightColor withBorder:YES withArrow:arrow forState:UIControlStateHighlighted];
  
  [nextButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
  [nextButton setTitleShadowColor:[UIColor colorWithRed:.25 green:.25 blue:.25 alpha:1] forState:UIControlStateNormal];
...
+ (void)drawGlossyRect:(CGRect)rect withColor:(UIColor*)color withArrow:(BOOL)arrow inContext:(CGContextRef)context
{
  ...
  if (arrow) {
    CGFloat x = CGRectGetMaxX(rect) - 10;
...
- (void)setBackgroundToGlossyRectOfColor:(UIColor*)color withBorder:(BOOL)border withArrow:(BOOL)arrow forState:(UIControlState)state
{
  ...
  if (border)
  {
    ...
    // Draw glossy image
    [UIButton drawGlossyRect:CGRectMake(1, 1, size.width-2, size.height-2) withColor:color withArrow:arrow inContext:context];
  }
  else
  {
    // Prepare clipping region
    ...
    // Draw glossy image
    [UIButton drawGlossyRect:CGRectMake(0, 0, size.width, size.height) withColor:color withArrow:arrow inContext:context];
  }
